...
async function loadData(){const r=await fetch('../data/products.json?v='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error('Não foi possível ler o catálogo.');products=await r.json();pendingByProduct={};try{const saved=JSON.parse(localStorage.getItem('amora_fut_admin_drafts_v1')||'{}');for(const [id,d] of Object.entries(saved)){if(d?.product&&products.some(p=>p.id===id))pendingByProduct[id]={product:d.product,files:[]};}}catch(e){}renderList();updateStats();if(typeof renderSaleProducts==='function')renderSaleProducts();if(typeof renderSales==='function')renderSales();}
...
$('#photos').addEventListener('change',e=>{pendingFiles=[...pendingFiles,...[...e.target.files].map(file=>({file,url:URL.createObjectURL(file)}))];if(currentId)captureDraft();renderPreviews((products.find(x=>x.id===currentId)||{}).images||[]);e.target.value='';});
