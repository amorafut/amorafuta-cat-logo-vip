...
let products=[],currentId=null,pendingFiles=[];
...
function blank(){return {id:'',team:'',name:'',category:'Brasileiras',version:'Torcedor',price:'',oldPrice:'',badge:'',description:'',published:true,soldOut:false,stock:{P:0,M:0,G:0,GG:0},images:[]};}
function setForm(p){
 currentId=p.id||null;$('#emptyEditor').classList.add('hidden');$('#form').classList.remove('hidden');$('#editMode').textContent=p.id?'EDITAR PRODUTO':'NOVO PRODUTO';$('#formTitle').textContent=p.name||'Novo produto';
...
async function publish(){
 ...
 $('#publish').disabled=true;msg('Publicando...');try{
  ...
  if(p&&pendingFiles.length){
   const imgs=[];for(let i=0;i<pendingFiles.length;i++){const path='assets/images/catalog/'+p.id+'/'+String(i+1).padStart(2,'0')+'.jpg';await publishFile(path,await b64File(pendingFiles[i].file));imgs.push(path);}p.images=imgs;
  }
  ...
  pendingFiles=[];renderPreviews((p&&p.images)||[]);renderList();updateStats();
...
$('#newProduct').onclick=newProduct;$('#reload').onclick=loadData;$('#filter').oninput=renderList;$('#publish').onclick=publish;$('#cancel').onclick=()=>{if(currentId)edit(currentId);else $('#form').classList.add('hidden');};
