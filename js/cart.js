...
function saveCart(c){localStorage.setItem(CART_KEY,JSON.stringify(c));clearFreightQuote();renderCart();if(freightCep.length===8&&c.length)quoteFreight()}
...
function setFreightCep(v){freightCep=String(v||'').replace(/[^\d]/g,'').slice(0,8);try{localStorage.setItem(CEP_KEY,freightCep)}catch(e){}const input=document.getElementById('freightCep');if(input){const digits=freightCep;input.value=digits.length>5?digits.slice(0,5)+'-'+digits.slice(5):digits;}clearFreightQuote()}
...
function renderFreight(){
 ...
 box.innerHTML='<div class="freight-title">CALCULAR ENTREGA</div><label class="freight-label" for="freightCep">CEP de destino</label><div class="freight-row"><input id="freightCep" inputmode="numeric" maxlength="9" placeholder="00000-000" value="'+safeText(freightCep.length>5?freightCep.slice(0,5)+'-'+freightCep.slice(5):freightCep)+'" oninput="setFreightCep(this.value)" onchange="setFreightCep(this.value)" onblur="setFreightCep(this.value)" aria-label="CEP de destino"><button type="button" class="freight-calc" onclick="quoteFreight()" '+(freightLoading?'disabled':'')+'>'+(freightLoading?'Calculando…':'Calcular frete')+'</button></div>'+(freightError?'<p class="freight-error">'+safeText(freightError)+'</p>':'')+options;
}
function renderCart(){
