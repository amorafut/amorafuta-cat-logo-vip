...
function setFreightCep(v){freightCep=String(v||'').replace(/\\D/g,'').slice(0,8);clearFreightQuote()}
...
function renderFreight(){
 ...
 box.innerHTML='<div class="freight-title">CALCULAR ENTREGA</div><label class="freight-label" for="freightCep">CEP de destino</label><div class="freight-row"><input id="freightCep" inputmode="numeric" maxlength="9" placeholder="00000-000" value="'+safeText(freightCep)+'" oninput="setFreightCep(this.value)" aria-label="CEP de destino"><button type="button" class="freight-calc" onclick="quoteFreight()" '+(freightLoading?'disabled':'')+'>'+(freightLoading?'Calculando…':'Calcular frete')+'</button></div>'+(freightError?'<p class="freight-error">'+safeText(freightError)+'</p>':'')+options;
}
function renderCart(){
