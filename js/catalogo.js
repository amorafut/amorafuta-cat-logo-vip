...
function card(p){
 ...
 return '<article class="card '+(p.soldOut?'sold':'')+'"><a class="card-open" href="produto.html?id='+encodeURIComponent(p.id)+'" aria-label="Ver '+esc(p.name)+'"><div class="photo">'+visual(p)+'<div class="placeholder"><div class="shirt-mark">'+initials(p.name)+'</div><span>AMORA FUT</span><small>STREETWEAR</small></div>'+(badge?'<b class="badge '+(p.soldOut?'sold-badge':'')+'">'+esc(badge)+'</b>':'')+(p.version?'<b class="version-badge '+(/jogador/i.test(p.version)?'version-player':(/torcedor/i.test(p.version)?'version-fan':''))+'">VERSÃO '+esc(p.version)+'</b>':'')+'<div class="quick">VER DETALHES</div></div><div class="info"><div class="catline">'+esc(p.team||p.category)+' <i>•</i> '+esc(p.version||'')+'</div><h3>'+esc(p.name)+'</h3><div class="price">'+(p.oldPrice?'<del>'+fmt(p.oldPrice)+'</del>':'')+'<strong>'+fmt(p.price)+'</strong></div><div class="pix"><span>PIX</span> '+fmt(pix(p.price))+'</div>'+(p.stock&&Object.keys(p.stock).length?'<div class="stock-card">'+(Object.values(p.stock).reduce((a,v)=>a+Number(v||0),0)>0?'<b>ESTOQUE: '+Object.values(p.stock).reduce((a,v)=>a+Number(v||0),0)+' UN.</b>':'<b class="stock-zero">ESGOTADO</b>')+'</div>':'')+'<span class="cta">Ver produto <b>→</b></span></div></a></article>';
}
function render(){
